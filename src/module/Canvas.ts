...
export default class Canvas {
  private _component: HTMLCanvasElement
  ...
  erase() {
    this._context?.clearRect(0, 0, this._component.width, this._component.height)
  }
  ...
  drawLine([startx, starty, endx, endy]: [number, number, number, number], props?: ILineProps) {
  ...
  drawRect([startx, starty, endx, endy]: [number, number, number, number], props?: IRectProps) {
  ...
  drawCircle([startx, starty, radius, startAngle = 0, endAngle = 2 * Math.PI]: [number, number, number, number, number], props?: ICircleProps) {
  ...
  drawText([text, startx, starty]: [string, number, number], props?: ITextProps) {
  ...
  translate(startx: number, starty: number) {
